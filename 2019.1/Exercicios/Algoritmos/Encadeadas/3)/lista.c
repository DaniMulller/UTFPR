...
void liga_lista(Lista *a,Lista *b){
  Celula *ultimaA = a->primeira;
  Celula *primeiraB = b->primeira;
  while(ultimaA->prox !=  NULL){
    ultimaA = ultimaA->prox;
  }
  ultimaA->prox = primeiraB;
  primeiraB->ant = ultimaA;
  free(b);
}

void exercicio3(Lista *A,Lista *B,Lista *C){
  Celula *aux = A->primeira;
  if(!verifica_lista_vazia(A)){
    while(aux != NULL){
      if(aux->item.chave % 2 == 0){
        inserir_inicio(B,aux->item.chave);
      }
      else{
        inserir_inicio(C,aux->item.chave);
      }
      aux = aux->prox;
    }
  }
